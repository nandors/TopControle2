              ...
              <v-layout wrap>
                ...
                <v-flex xs12 sm4 md4>
                  <v-text-field
                    name='txtrecebido'
                    label='recebido'
                    :rules="[ v => !!v || 'recebido requerido']"
                    id='txtrecebido'
                    type='text'
                    v-model='newRegistro.recebido'
                    required></v-text-field>
                </v-flex>
                ...
                <v-flex xs12 sm4 md4>
                  <v-text-field
                    name='txtpedidonr'
                    label='pedidonr'
                    :rules="[ v => !!v || 'pedidonr requerido']"
                    id='txtpedidonr'
                    type='text'
                    v-model='newRegistro.pedidonr'
                    required></v-text-field>
                </v-flex>
                ...
                <v-flex xs12 sm4 md4>
                  <v-text-field
                    name='txtordem'
                    label='ordem'
                    :rules="[ v => !!v || 'ordem requerido']"
                    id='txtordem'
                    type='text'
                    v-model='newRegistro.ordem'
                    required></v-text-field>
                </v-flex>
      ...
      <v-card>
        ...
        <v-data-table
          ...
          <template slot="items" slot-scope="props">
              ...
              <td class='text-xs-left'>{{ props.item.recebido }}</td>
              <td class='text-xs-left'>{{ props.item.pedido }}</td>
              <td class='text-xs-left'>{{ props.item.saldo }}</td>
              <td class='text-xs-left'>{{ props.item.pedidonr }}</td>
              <td class='text-xs-left'>{{ props.item.ordem }}</td>
    ...
    data: function () {
      return {
        headers: [
          ...
          { text: 'recebido', align: 'left', value: 'recebido' },
          { text: 'pedido', align: 'left', value: 'pedido' },
          { text: 'saldo', align: 'left', value: 'saldo' },
          { text: 'pedidonr', align: 'left', value: 'pedidonr' },
          { text: 'ordem', align: 'left', value: 'ordem' }
        ],
        newRegistro: {
          codigo: '',
          data: '',
